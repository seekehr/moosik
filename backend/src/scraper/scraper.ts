...
interface Scraper {
    scrapeAlbum(url: string): Promise<void>;
}
...
export class SpotifyScraper implements Scraper {
    async scrapeAlbum(url: string = 'https://open.spotify.com/album/0fSfkmx0tdPqFYkJuNX74a'): Promise<void> {
        ...
        try {
            ...
            if (trackInfos.length > 0 && trackInfos.every(track => track.duration === 'Unknown')) {
                ...
                const durations = await page.$$eval('div[role="row"] [aria-colindex="5"], div[role="row"] div.Type__TypeElement-sc-goli3j-0', 
